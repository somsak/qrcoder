...
public final class ShareActivity extends Activity {
	...
	private final Button.OnClickListener bookmarkListener = new Button.OnClickListener() {
		public void onClick(View v) {
			Intent intent = new Intent(Intent.ACTION_PICK);
			intent.addFlags(Intent.FLAG_ACTIVITY_CLEAR_WHEN_TASK_RESET);
			intent.setClassName(ShareActivity.this, BookmarkPickerActivity.class
					.getName());
	...
	private final Button.OnClickListener manualListener = new Button.OnClickListener() {
		public void onClick(View v) {
			...
			intent.setClassName(ShareActivity.this, CustomQRActivity.class.getName());
	...
	public void onCreate(Bundle icicle) {
		...
		findViewById(R.id.yp_contact_button).setOnClickListener(contactListener);
		findViewById(R.id.yp_bookmark_button).setOnClickListener(bookmarkListener);
	 ...
	 * that person's info, then sends an Encode intent to render it as a QR Code.
	 * 
	 * @param contactUri
	...
	private void showContactAsBarcode(Uri contactUri) {
		...
		if (contactCursor != null && contactCursor.moveToFirst()) {
			...
			if (name != null && name.length() > 0) {
				bundle
						.putString(Contacts.Intents.Insert.NAME, massageContactData(name));
			}
			contactCursor.close();

			Uri phonesUri = Uri.withAppendedPath(contactUri,
					Contacts.People.Phones.CONTENT_DIRECTORY);
			Cursor phonesCursor = resolver.query(phonesUri, PHONES_PROJECTION, null,
					null, null);
			...
			Cursor methodsCursor = resolver.query(methodsUri, METHODS_PROJECTION,
					null, null, null);
